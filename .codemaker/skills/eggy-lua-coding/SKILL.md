...
- **必须**使用提供的文件写入工具来确保写入的代码是 UTF-8 编码的，**禁止**使用其他任何方式（例如依赖命令行）尝试修改代码。
- 修改统一使用严格匹配并替换的形式：
...
- 如果单一文件有多处修改，不要包含不修改的部分整体替换，已保留的代码为分界线逐块替换修改。
- 如果需要新建文件，则在 `<exact-content-to-replace>` 留空并要求创建新的文件。
